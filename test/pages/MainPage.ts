...
export class MainPage {
    ...
    async addTransaction(transaction: Transaction): Promise<void> {
        try {
            ...
            await browser.pause(1000);
            await timeSelectionPopup.moveToMonth(transaction.date.split('-')[1], transaction.date.split('-')[0]);
            await browser.pause(1000);
            await timeSelectionPopup.dayInput(transaction.date.split('-')[2]).click();
            await browser.pause(1000);
